...
namespace ICEProject
{
    ...
    {
        //==== Pointer ==== (GÜNCEL - 2025 Ocak 10)
        public const int KO_PTR_CHR = 0x010F5FE0;                    // Ana karakter pointer - UPDATED!
        public const int KO_PTR_DLG = 0x010F6094;                    // Dialog/UI pointer - UPDATED!
        public const int KO_PTR_PKT = 0x010F60AC;                    // Paket pointer - UPDATED!
        public const int KO_SND_FNC = 0x00701660;                    // Paket gönderme fonksiyonu - UPDATED!
        public const int KO_FMBS = 0x004E1480;                       // Field Mob Base Structure (Map_Next_Item) - UPDATED!
        public const int KO_FPBS = 0x004E1480;                       // Field Player Base Structure (Map_Next_Item_v2) - UPDATED!
        public const int KO_FLDB = 0x010F5FEC;                       // Field Layer DB (PlayerOtherMgr) - UPDATED!
        public const int KO_FNC_SEL = 0x0080D4B0;                    // Select fonksiyonu - ADDED!
        public const int KO_FNC_ISEN = 0x558260;                     // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_CHAR_SERV = 0xC397EC;                    // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_OTO_LOGIN_PTR = 0xDE85F8;                // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_OTO_LOGIN_01 = 0x4D56D0;                 // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_OTO_LOGIN_02 = 0x4CEBA0;                 // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_OTO_LOGIN_03 = 0x4CE660;                 // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_OTO_LOGIN_04 = 0x4D2FD0;                 // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_OTO_BTN_PTR = 0xDE8618;                  // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_BTN_LEFT = 0x4BE5E0;                     // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_BTN_RIGHT = 0x4BE880;                    // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_BTN_LOGIN = 0x4BAE10;                    // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_FNCZ = 0x51B7C0;                         // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_FNCB = 0x51B930;                         // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_ITOB = 0xDF8618;                         // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_RECV_FNC = 0x53B290;                     // [ESKİ - KONTROL EDİLMELİ]
        public const int KO_RECV_PTR = 0xB48038;                     // [ESKİ - KONTROL EDİLMELİ]

        // GÜNCEL POINTER'LAR (2025 Ocak)
        public const int KO_PTR_N3WorldManager = 0x010F5FDC;        // World Manager
        public const int KO_PTR_Table_UPC_Skill = 0x010F5FC8;       // Skill Table
        public const int KO_PTR_LOCAL_INPUT = 0x010F60B4;           // Local Input
        public const int KO_PTR_CGameProcIntroChrSelect = 0x010F6098; // Character Select - ADDED!
        public const int KO_PTR_GameProcIntro = 0x010F609C;         // Game Intro - ADDED!
        public const int KO_PTR_GameProcLogo = 0x010F60A0;          // Game Logo - ADDED!
        public const int KO_PTR_RECV1 = 0x010F60A8;                 // Receive Handler - ADDED!
        public const int KO_PTR_TABLE_ITEM_BASIC = 0x010F5EF4;      // Item Table - ADDED!
        public const int KO_PTR_PASSWORD = 0x010E0134;              // Password Buffer - ADDED!
        public const int KO_PTR_CCountableItemEditDlg = 0x010F6DDC; // Item Edit Dialog - ADDED!
        public const int KO_PTR_CN3UIWndBase = 0x011C6B3C;          // UI Window Base - ADDED!
        public const int KO_PTR_SMMB = 0x00000000;                  // Skill Manager - PATTERN GEREKLİ!
        public const int KO_FNC_SMMB = 0x00000000;                  // Skill fonksiyonu - PATTERN GEREKLİ!
        public const int KO_FNC_Legal_Attack = 0x005A89E0;          // Legal Attack Function - UPDATED!
        public const int KO_LEGALSKILL = 0x00880970;                // Legal Skill (LSCA) - UPDATED!
        public const int KO_FNC_CharacterSelectOrCreate = 0x007C3ED0; // Char Select - ADDED!
        public const int KO_FNC_Encryption = 0x005E65C0;            // Encryption - ADDED!
        public const int KO_FNC_NPC_IS_ENEMY = 0x007952B0;          // NPC Enemy Check - ADDED!
        public const int KO_FNC_NPC_IS_ENEMY_v2 = 0x007952B0;       // NPC Enemy Check v2 - ADDED!
        public const int KO_FNC_LOGIN1 = 0x007BA6A0;                // Login Function 1 - ADDED!
        public const int KO_FNC_LOGIN2 = 0x00701250;                // Login Function 2 - ADDED!
        public const int KO_FNC_GetChildAreaByiOrder = 0x008D0BA0;  // Get Child Area - ADDED!
        public const int KO_SBEC = 0x005038C0;                      // Send Buffer Execute - ADDED!

        //==== Offsetler ==== (GÜNCEL - 2025 Ocak 10)
        public const int KO_OFF_CLASS = 0x6CC;                       // Sınıf (Warrior/Rogue/Mage/Priest)
        public const int KO_OFF_NATION = 0x6C4;                      // Nation (Karus/Human) - RENAMED!
        public const int KO_OFF_NT = 0x6C4;                          // Nation (alias - backward compat)
        public const int KO_OFF_MOVE = 0xF90;                        // Hareket durumu
        public const int KO_OFF_e_StateMove = 0x140;                 // e_StateMove - RENAMED!
        public const int KO_OFF_MOVEType = 0x140;                    // (alias - backward compat)
        public const int KO_OFF_GoX = 0xF9C;                         // Hedef X koordinatı
        public const int KO_OFF_GoZ = 0xFA0;                         // Hedef Z koordinatı
        public const int KO_OFF_GoY = 0xFA4;                         // Hedef Y koordinatı
        public const int KO_OFF_POSX = 0x3CC;                        // Mevcut X pozisyonu - RENAMED!
        public const int KO_OFF_X = 0x3CC;                           // (alias - backward compat)
        public const int KO_OFF_POSZ = 0x194;                        // Mevcut Z pozisyonu - RENAMED!
        public const int KO_OFF_Z = 0x194;                           // (alias - backward compat)
        public const int KO_OFF_POSY = 0x3D4;                        // Mevcut Y pozisyonu - RENAMED!
        public const int KO_OFF_Y = 0x3D4;                           // (alias - backward compat)
        public const int KO_OFF_ID = 0x6A0;                          // Karakter/Mob ID
        public const int KO_OFF_NAME = 0x6A4;                        // İsim pointer'ı
        public const int KO_OFF_TARGET = 0x660;                      // Seçili hedef ID
        public const int KO_OFF_WH = 0x6DC;                          // Wall Hack
        public const int KO_OFF_MCOR = 0x420;                        // Mouse Coordinates
        public const int KO_OFF_m_fYawToReach = 0x17C;               // Yaw Rotation - ADDED!

        public const int KO_OFF_LEVEL = 0x6D0;                       // Seviye - UPDATED!
        public const int KO_OFF_MAX_HP = 0x6D4;                      // Max HP - UPDATED!
        public const int KO_OFF_HP = 0x6D8;                          // Current HP - UPDATED!
        public const int KO_OFF_MAXHP = 0x6D4;                       // (alias - backward compat)
        public const int KO_OFF_MAX_MP = 0xBEC;                      // Max MP - UPDATED!
        public const int KO_OFF_MP = 0xBF0;                          // Current MP - UPDATED!
        public const int KO_OFF_MAXMP = 0xBEC;                       // (alias - backward compat)
        public const int KO_OFF_GOLD = 0xBFC;                        // Gold - UPDATED!
        ...
        public const int KO_OFF_SKILL_PTS_ARCHER = 0x174;            // Archer Skill Points - ADDED!
        public const int KO_OFF_SKILL_PTS_ASSASSIN = 0x178;          // Assassin Skill Points - ADDED!
        public const int KO_OFF_SKILL_PTS_SEARCH = 0x17C;            // Search Skill Points - ADDED!
        public const int KO_OFF_SKILL_PTS_MASTER = 0x180;            // Master Skill Points - ADDED!
        ...
        public const int KO_OFF_SkillBase = 0x210;                   // Skill Base - UPDATED!
        public const int KO_OFF_SKILLBASE = 0x210;                   // (alias - backward compat)
        public const int KO_OFF_SKILLID = 0x12C;
        
        //==== UI Offsetleri ==== (ADDED)
        public const int KO_OFF_UILoginIntro = 0x2C;                 // Login UI Intro
        public const int KO_OFF_UI_EDIT_ID = 0x118;                  // Username Edit Box
        public const int KO_OFF_UI_EDIT_PASS = 0x11C;                // Password Edit Box
        public const int KO_OFF_UI_EDIT_VALUE = 0x134;               // Edit Value
        public const int KO_OFF_UI_CONNECT_BUTTON = 0x120;           // Connect Button
        public const int KO_OFF_UI_VISIBLE = 0xEA;                   // UI Visibility
        public const int KO_OFF_FIRST_SERVER = 0x254;                // First Server
        public const int KO_OFF_SERVER_OFFSET = 0x1C;                // Server Offset
        public const int KO_OFF_CH_LIST = 0x480;                     // Channel List
        //==== İtem Silme ====
        ...
        /// <summary>
        /// Paket gönderir (GÜNCEL pointer: 0x010F60D8)
        /// </summary>
        public void setPack(string Packet)
        {
            ...
            string pCode = "60";                                    // PUSHAD
            pCode += "8B0D" + Adword(KO_PTR_PKT, 4);              // MOV ECX, [KO_PTR_PKT]
            pCode += "68" + Adword(Packet.Length / 2, 4);         // PUSH length
            pCode += "68" + Adword((int)KoSendAdress, 4);         // PUSH packet_addr
            pCode += "BF" + Adword(KO_SND_FNC, 4);                // MOV EDI, KO_SND_FNC
            pCode += "FFD7";                                       // CALL EDI
            pCode += "61";                                         // POPAD
            pCode += "C3";                                         // RET
        ...
        /// <summary>
        /// Karakterin X pozisyonu (GÜNCEL offset: 0x3CC)
        /// </summary>
        public int CharX()
        {
        ...
        /// <summary>
        /// Karakterin Y pozisyonu (GÜNCEL offset: 0x3D4)
        /// </summary>
        public int CharY()
        {
        ...
        /// <summary>
        /// Karakterin Z pozisyonu (GÜNCEL offset: 0x194)
        /// </summary>
        public int CharZ()
        {
        ...
        /// <summary>
        /// Karakterin CLASS ID'si (GÜNCEL offset: 0x6CC)
        /// </summary>
        ...
        /// <summary>
        /// Karakterin Nation'ı (GÜNCEL offset: 0x6C4)
        /// </summary>
        public int CharNation()
        {
            return getLong(getLong(KO_PTR_CHR) + KO_OFF_NT);
        }
        ...
        {
            return getLong(getLong(KO_PTR_CHR) + KO_OFF_TARGET);
        }
        ...
        public void setSelectMob(int TargetID,bool ManuelMob = false)
        {
            if (TargetID <= 0)
                return;
            setLong(getLong(KO_PTR_CHR) + KO_OFF_TARGET, TargetID);
            if(ManuelMob)
            {
        ...
        /// <summary>
        /// En yakın hedefi bulur ve seçer (GÜNCEL - GetBase ile optimize)
        /// </summary>
        public void getZTarget(string TargetStatu, string TargetControl = "Genel", ListBox lstTarget = null, bool hpControl = false)
        {
            ...
            while (Esi != Ebp)
            {
                // 50ms timeout (GÜNCEL - eski 50ms'den değişmedi ama kontrol iyileştirildi)
                if (GetTickCount() - GetTime > 50) break;
                ...
                switch (TargetControl)
                {
                    ...
                    case "List":
                        if (searchList(getTargetName(Base_Adr, true), lstTarget))
                        {
                            if (CrrDist < LDist && getTargetStatu(Base_Adr) != 10)
                            {
                                LID = getLong(Base_Adr + KO_OFF_ID);
        ...
        #endregion
        //#########################################################################################
        #region [WidexSource Entegrasyonu - Atak ve Loot Lojikleri]

        /// <summary>
        /// GetBase - ID'ye göre Player veya Mob base pointer'ını döndürür
        /// C++ SnoxdTr versiyonunun C# implementasyonu
        /// ID > 5000 = Mob/NPC, ID < 5000 = Player
        /// </summary>
        public int GetBase(int ID)
        {
            try
            {
                // ID'ye göre hangi fonksiyonu kullanacağımıza karar ver
                // DÜZELTME: 10000 yerine 5000 eşiği kullan (C++ koduna uygun)
                int getFuncAddress = (ID > 5000) ? KO_FMBS : KO_FPBS;
                if (getFuncAddress == 0) return 0;

                // Assembly: FMBS/FPBS çağrısı
                // C++ ASM:
                //   mov ecx, KO_FLDB
                //   mov ecx, [ecx]
                //   push 1
                ...
                //   call edi
                string pCode = "8B0D" + Adword(KO_FLDB, 4);      // MOV ECX, [KO_FLDB]
                pCode += "6A01";                                 // PUSH 1
                pCode += "68" + Adword(ID, 4);                   // PUSH ID
                pCode += "BF" + Adword(getFuncAddress, 4);       // MOV EDI, getFuncAddress
                pCode += "FFD7";                                 // CALL EDI
                pCode += "A3" + Adword((int)KoExecuteAdress, 4); // MOV [KoExecuteAdress], EAX
                pCode += "C3";                                   // RET
                exCodeHandle(pCode);
                return getLong((int)KoExecuteAdress);
            }
            catch { return 0; }
        }
        ...
        {
            // Dahili "Z" fonksiyonu (FNCZ)
            string pCode = "8B0D" + Adword(KO_PTR_DLG, 4);
            pCode += "B8" + Adword(KO_FNCZ, 4);
            pCode += "FFD0";
            pCode += "C3";
            exCodeHandle(pCode);
        }
        ...
        /// <summary>
        /// LegalAttack - XignCode bypass'li legal skill attack
        /// C++ Private Bot versiyonunun C# implementasyonu
        /// NOT: KO_PTR_SMMB ve KO_FNC_SMMB pattern ile bulunmalı!
        /// </summary>
        public void LegalAttack(int skillID, int targetID)
        {
            // Eğer SMMB pointer'ları yoksa basit versiyonu kullan
            if (KO_PTR_SMMB == 0 || KO_FNC_SMMB == 0)
            {
                // Basit versiyon (eski)
                if (KO_LEGALSKILL == 0) return;
                string pCodeBasic = "60";                                     // PUSHAD
                pCodeBasic += "68" + Adword(targetID, 4);                   // PUSH targetID
                pCodeBasic += "68" + Adword(skillID, 4);                    // PUSH skillID
                pCodeBasic += "B8" + Adword(KO_LEGALSKILL, 4);              // MOV EAX, KO_LEGALSKILL
                pCodeBasic += "FFD0";                                        // CALL EAX
                pCodeBasic += "61";                                          // POPAD
                pCodeBasic += "C3";                                          // RET
                exCodeHandle(pCodeBasic);
                return;
            }

            // Gelişmiş versiyon (C++ Private Bot)
            // C++ ASM:
            //   mov ecx, [KO_PTR_SMMB]
            ...
            //   call edi
            string pCode = "8B0D" + Adword(KO_PTR_SMMB, 4);           // MOV ECX, [KO_PTR_SMMB]
            pCode += "8B09";                                           // MOV ECX, [ECX]
            pCode += "68" + Adword(skillID, 4);                       // PUSH skillID
            pCode += "BF" + Adword(KO_FNC_SMMB, 4);                   // MOV EDI, KO_FNC_SMMB
            pCode += "FFD7";                                           // CALL EDI
            pCode += "8B0D" + Adword(KO_PTR_DLG, 4);                  // MOV ECX, [KO_PTR_DLG]
            pCode += "8B09";                                           // MOV ECX, [ECX]
            pCode += "8B89" + Adword(0x448, 4);                       // MOV ECX, [ECX+0x448]
            pCode += "50";                                             // PUSH EAX
            pCode += "68" + Adword(targetID, 4);                      // PUSH targetID
            pCode += "BF" + Adword(KO_LEGALSKILL, 4);                 // MOV EDI, KO_LEGALSKILL
            pCode += "FFD7";                                           // CALL EDI
            pCode += "C3";                                             // RET
            exCodeHandle(pCode);
        }
        ...
        public void Archer3Arrow(int targetID)
        {
            int skillID = getSkillID(CharClass(), "515");
            string targetHex = Adword(targetID, 2);
            string charHex = CharID();
            string sID = Adword(skillID, 3); // 3-byte skill ID common in archery

            setPack("3101" + sID + "00" + charHex + targetHex + "00000000000000000000000000000D00");
            setPack("3102" + sID + "00" + charHex + targetHex + "000000000000010000000000");

            for (int i = 1; i <= 3; i++)
            {
                setPack("3103" + sID + "00" + charHex + targetHex + "0000000000000" + i + "0000000000000000");
            }
        }

        public void Archer5Arrow(int targetID)
        {
            int skillID = getSkillID(CharClass(), "555");
            string targetHex = Adword(targetID, 2);
            string charHex = CharID();
            string sID = Adword(skillID, 3);

            setPack("3101" + sID + "00" + charHex + targetHex + "00000000000000000000000000000F00");
            setPack("3102" + sID + "00" + charHex + targetHex + "000000000000010000000000");
        ...
        public void AsasAttack(int targetID, string skillSuffix)
        {
            int skillID = getSkillID(CharClass(), skillSuffix);
            string targetHex = Adword(targetID, 2);
            string charHex = CharID();

            // Spike, Trust vb. için ortak lojik
            setPack("3101" + Adword(skillID, 4) + charHex + targetHex + "00000000000000000000000000001000");
            setPack("3103" + Adword(skillID, 4) + charHex + targetHex + "000000000000000000000000");
        }

        public void LootOpen(int boxID)
        {
            setPack("24" + Adword(boxID, 4));
        }

        public void GetLootItem(int boxID, int itemID, int slot)
        {
            setPack("26" + Adword(boxID, 4) + Adword(itemID, 4) + Adword(slot, 1) + "00");
        }

        public int GetNearMobs(int range)
        {
            int lastDist = range;
            int lastMob = -1;
            for (int i = 10000; i < 32767; i++)
            {
                int mobBase = GetBase(i);
                if (mobBase != 0)
                {
                    if (getLong(mobBase + KO_OFF_HP) > 0)
                    {
                        float mX = getFloat(mobBase + KO_OFF_X);
                        float mY = getFloat(mobBase + KO_OFF_Y);
                        int dist = getDistance(CharX(), CharY(), (int)mX, (int)mY);
                        if (dist < lastDist) { lastDist = dist; lastMob = i; }
                    }
                }
            }
            return lastMob;
    ...
    /// <summary>
    /// Otomatik pointer bulma sistemi (Pattern Scanning)
    /// Oyun güncellendiğinde pointer'ları otomatik bul
    /// </summary>
    public class PointerScanner
    {
        private cls_Bot bot;

        public PointerScanner(cls_Bot botInstance)
        {
            bot = botInstance;
        }

        /// <summary>
        /// KO_FMBS pointer'ını pattern ile bulur
        /// Pattern: "CCCCCCCCCC83ECXX5356578D71"
        /// </summary>
        public int FindFMBS()
        {
            try
            {
                ...
                0x83, 0xEC, 0xFF, // 0xFF = wildcard
                0x53, 0x56, 0x57, 0x8D, 0x71 
            };

                // 0x400000 - 0x800000 arası tara (code section)
                for (int addr = 0x400000; addr < 0x800000; addr += 0x1000)
                {
                    if (MatchPattern(addr, pattern))
                    {
                        return addr + 0x8; // CallOffset
                    }
                }
            }
            catch { }
        ...
        /// <summary>
        /// KO_FPBS pointer'ını pattern ile bulur
        /// Pattern: "83EC085356578D71XX8D44241850"
        /// </summary>
        public int FindFPBS()
        {
            try
            {
                byte[] pattern = new byte[] { 
                0x83, 0xEC, 0x08, 0x53, 0x56, 0x57, 
                0x8D, 0x71, 0xFF, // 0xFF = wildcard
                0x8D, 0x44, 0x24, 0x18, 0x50 
            };
        ...
        /// <summary>
        /// Pattern eşleştirme (0xFF = wildcard)
        /// </summary>
        private bool MatchPattern(int address, byte[] pattern)
        {
            try
            {
                for (int i = 0; i < pattern.Length; i++)
                {
                    byte b = (byte)bot.getByte(address + i);
                    if (pattern[i] != 0xFF && b != pattern[i])
                        return false;
                }
                return true;
            }
